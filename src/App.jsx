      ...
      <div className="relative bg-slate-900 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto py-24 px-4 sm:px-6 lg:px-8 flex flex-col items-start justify-center min-h-[600px]">
          ...
          <p className="mt-4 text-xl text-gray-300 max-w-2xl mb-8">
            SAR Network provides 24/7 rapid response towing and roadside assistance. Trusted by thousands of drivers nationwide.
          </p>
  ...
  useEffect(() => {
    const getLiveJobs = async () => {
      try {
        ...
        const fetched = [];
        snap.forEach(doc => fetched.push({ id: doc.id, ...doc.data() }));
        // Ensure the newest posted jobs are at the top of the list!
    ...
    return (
      <div className="max-w-md mx-auto py-16 px-4">
        <div className="bg-green-50 p-8 rounded-2xl text-center border border-green-200 shadow-sm">
          ...
          <button onClick={() => { setSelectedJob(null); setIsApplying(false); setApplicationStatus('idle'); }} className="text-green-700 font-bold hover:underline">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          <div className="bg-slate-900 px-8 py-10 text-white relative overflow-hidden">
            <div className="absolute top-0 right-0 opacity-10 pointer-events-none">
              <Icons.Briefcase className="w-64 h-64 -mt-10 -mr-10" />
            </div>
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-6">
                ...
                <span className="bg-green-500/20 text-green-400 text-sm px-4 py-1.5 rounded-full font-bold uppercase border border-green-500/30">Actively Hiring</span>
              </div>
              <h2 className="text-3xl sm:text-4xl font-extrabold mb-6">{selectedJob.title}</h2>
              <div className="flex flex-wrap items-center text-slate-300 gap-y-4 text-sm sm:text-base">
                <span className="flex items-center mr-8"><Icons.MapPin className="h-5 w-5 mr-2 text-sky-400" /> {selectedJob.loc}</span>
                <span className="flex items-center mr-8"><Icons.Clock className="h-5 w-5 mr-2 text-sky-400" /> {selectedJob.type}</span>
                <span className="flex items-center font-bold text-green-400"><span className="mr-1 text-green-500">$</span> {selectedJob.pay}</span>
              </div>
            </div>
          </div>

          <div className="p-8 sm:p-10">
            <h3 className="text-xl font-bold text-slate-900 mb-6 border-b border-gray-100 pb-4">Job Description & Requirements</h3>
            <div className="prose max-w-none text-slate-700 whitespace-pre-wrap leading-relaxed mb-12 text-lg">
              {selectedJob.desc || "We are looking for a dedicated professional to join our team. Apply below to learn more!"}
            </div>

            <div className="bg-sky-50 rounded-2xl p-8 border border-sky-100 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between">
               <div>
                 <h4 className="text-xl font-bold text-slate-900 mb-2">Ready to join the SAR Network?</h4>
                 <p className="text-slate-600 mb-4 sm:mb-0">Take the next step in your career. We review applications within 3-5 business days.</p>
               </div>
               <button
                 onClick={() => { setIsApplying(true); window.scrollTo(0,0); }}
                 className="w-full sm:w-auto px-8 py-4 bg-sky-600 text-white font-bold rounded-xl hover:bg-sky-700 transition-colors shadow-lg shadow-sky-200 shrink-0 text-lg flex items-center justify-center"
               >
                 Start Application <Icons.ChevronRight className="h-5 w-5 ml-2" />
               </button>
            </div>
          </div>
        </div>
  ...
  if (selectedJob && isApplying) {
    return (
      <div className="max-w-3xl mx-auto py-12 px-4 sm:px-6">
        <button onClick={() => setIsApplying(false)} className="flex items-center text-slate-500 hover:text-sky-600 font-medium mb-6 transition-colors">
          <Icons.ArrowLeft className="h-4 w-4 mr-2" /> Back to Job Details
        </button>
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div className="pt-4 border-t border-gray-100 flex items-center justify-end gap-4">
              <button type="button" onClick={() => setIsApplying(false)} className="px-6 py-3 text-slate-600 font-bold hover:bg-slate-100 rounded-lg">Cancel</button>
    ...
    <div className="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {isLoadingJobs ? (
          ...
          liveJobs.map((job) => (
            <div key={job.id} className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-all hover:-translate-y-1 flex flex-col">
              <div className="p-6 flex flex-col h-full">
                ...
                <button 
                  ...
                  className="w-full py-2.5 border-2 border-sky-600 text-sky-600 font-bold rounded-lg hover:bg-sky-50 transition-colors"
                >
                  View Details & Apply
                </button>
              </div>
          ...
          <button 
             onClick={() => { 
               setSelectedJob({ 
                 ...
                 type: "Full/Part-time", 
                 pay: "Competitive",
                 desc: "Don't see a specific role that matches your skills right now? We are always on the lookout for talented tow operators, dispatchers, technicians, and support staff to join our growing network.\n\nSubmit your general application and resume using the form below. Our hiring team will keep your information securely on file and reach out if a position opens up that fits your background and expertise."
               }); 
               setIsApplying(false);
               window.scrollTo(0, 0); 
             }}
...
// --- EMPLOYEE PORTAL (FIREBASE CONNECTED) ---
